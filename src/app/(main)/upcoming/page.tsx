...
export default function UpcomingPage() {
  ...
  const initialType = (searchParams.get('type') as UpcomingType) || 'Movie';
  ...
  const fetchContent = React.useCallback(async (type: UpcomingType, language: string, pageNum: number, initialLoad = false) => {
    if (initialLoad) {
      setIsLoading(true);
      setContent([]);
      setHasMore(true);
    ...
    try {
      const response = await fetch(`/api/movieseries/upcoming?type=${type}&languages=${language}&limit=10&page=${pageNum}`);
      const data = await response.json();
      if (data.success) {
        setContent(prev => pageNum === 1 ? data.data : [...prev, ...data.data]);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b pb-4 gap-4">
        <h1 className="text-3xl sm:text-4xl font-headline font-bold">Upcoming Releases</h1>
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {content.map((item) => (
              <MovieCard key={item._id} movie={{...item, movie_title: item.title, id: item._id} as Movie} />
            ))}
          </div>
          {hasMore && (
        ...
        <div className="text-center text-muted-foreground py-16">
          <h2 className="text-xl font-semibold">No upcoming {activeTab.toLowerCase()} found.</h2>
