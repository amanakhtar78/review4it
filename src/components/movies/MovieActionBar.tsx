...
export default function MovieActionBar({ movie }: MovieActionBarProps) {
      ...
      >
        <Icon
          className={cn(
            ...
            isSet ? "text-primary fill-primary" : "text-muted-foreground"
          )}
        />
      </Button>
  ...
  const buttons = (
    <>
      <ActionButton
        actionType="like"
        isSet={isMovieLiked(movieId!)}
        count={counts.likes}
        Icon={Heart}
        className="hover:bg-rose-100 dark:hover:bg-rose-900/50"
      />
      <ActionButton
        actionType="save"
        isSet={isMovieSaved(movieId!)}
        count={counts.saves}
        Icon={Bookmark}
        className="hover:bg-sky-100 dark:hover:bg-sky-900/50"
      />
      <ActionButton
        actionType="dislike"
        isSet={isMovieDisliked(movieId!)}
        count={counts.dislikes}
        Icon={ThumbsDown}
        className="hover:bg-slate-200 dark:hover:bg-slate-700/50"
      />
    </>
  );
    ...
    return (
      <Dialog>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Join Review4it!</DialogTitle>
            <DialogDescription>
