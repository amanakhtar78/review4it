...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    const isFullCustomButton = !asChild && !isIconOnly && variant === "default" && size === "default"; // Example: only default variant/size gets full custom

    if (isFullCustomButton) {
      // This button gets the .custom-cinefolio-button class which includes its specific shadow, border, etc.
      return (
        <Comp
          className={cn(buttonVariants({ variant, size }), "custom-cinefolio-button", className)}
          ref={ref}
          {...props}
        >
          ...
          <div className="arrowContainer">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M5 12H19"/>
